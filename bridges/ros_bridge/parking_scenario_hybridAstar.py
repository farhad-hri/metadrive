...
from HybridAStar.hybrid_a_star import HA_planner
...
ego_state['position'][0], ego_state['position'][1] = 20.0, 0.0
...
s = [ego_state['position'][0], ego_state['position'][1], ego_state['heading_theta']]
start = [s[0] - d*np.cos(s[2]), s[1] - d*np.sin(s[2]), s[2]]
goal_c = [center_spots[8][0], center_spots[8][1], -np.pi/2.0]
goal_yaw = goal_c[2]
goal = [goal_c[0]- d*np.cos(goal_yaw), goal_c[1]- d*np.sin(goal_yaw), goal_yaw]
obstacleX=[0.0]
obstacleY=[0.0]
...
for i in range(1, 1000000):
    ...
    v = result[0].vels[int(i*0.1)]
    steering = result[0].steers[int(i*0.1)]
    heading = current_heading + (v/HA_obj.Car.wheelBase)*np.tan(steering)*dt
    vel_global = np.array([v*np.cos(heading), v*np.sin(heading)]) 
